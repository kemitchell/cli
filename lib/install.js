...
Installer.prototype.printInstalledForHuman = function (diffs, auditResult) {
  ...
  var updated = 0
  var moved = 0
  // Check if any installed packages have funding properties.
  var haveFunding = false
  ...
  diffs.forEach(function (action) {
    ...
    if (mutation !== 'remove' && pkg.package.funding) {
      haveFunding = true
    }
    if (mutation === 'remove') {
      ++removed
  ...
  output(report)
  if (haveFunding) {
    output('Run `npm fund` to support the projects you rely on.')
  }
  if (auditResult) {
